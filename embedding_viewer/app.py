...
def load_embeddings(path: Path):
    ...
    if path.suffix == ".jsonl":
        ...
        return np.array(vecs, np.float32), np.array(labels), None

    raise ValueError("JSONL only for now")
...
def plot_embedding(X2d, labels):
    ...
    for lab in uniq:
        m = labels == lab
        plt.scatter(X2d[m,0], X2d[m,1], s=14, alpha=0.7, label=f"{lab}")
