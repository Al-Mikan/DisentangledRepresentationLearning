...
CONFIG: Dict[str, object] = {
    ...
    "LOSS_FUNCTION": ["triplet", "improved"],
    ...
    "USE_24FPS": True,
    "INTERACTIVE": False, 
    "DATASET_NAME": "animalkingdom_50",  # "animalkingdom" | "wolf" など
}
# ====== 2. ユーティリティ ======
...
def visualize(
    ...
    for method in cfg["VISUALIZE"]:
        ...
        ax.set_title(f"{method.upper()} ({vis_out_base})\nARI={ari:.4f} / NMI={nmi:.4f}")
    ...
    for mode in cfg["VECTOR_MODES"]:
        for loss in cfg["LOSS_FUNCTION"]:
            for suffix in cfg["SUFFIXES"]:
                ...
                # 4) 評価 & 可視化
                out_dir = Path("result") / cfg["DATASET_NAME"] / cfg["VMAE_VERSION"] / cfg["DATA_MODE"]
                ensure_dirs(out_dir)
