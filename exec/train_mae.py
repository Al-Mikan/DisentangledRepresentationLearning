...
def train_one(loss_type, mode, use_grl=True, use_mlp=False, datatype='animalkingdom', vmae_version='base'):
    ...
    # ---- ログ & パス ----
    path_suffix = f"{'mlp' if use_mlp else 'linear'}-{'grl' if use_grl else 'nogrl'}-adv{LAMBDA_ADV:.2f}"
    suffix = f"{datatype}_{vmae_version}_{mode}_{loss_type}_{path_suffix}"

    dir_model = f"./models/model_mae/{datatype}/{vmae_version}/{loss_type}"
    dir_loss = f"./losses/loss_mae/{datatype}/{vmae_version}/{loss_type}"
...
def main():
    try:
        vmae_version = "base" # "base", "v2_base", "v2_large"
        all_modes = ['sliding'] # 'simple', 'adaptive3d', 'adaptive1d', 'sliding'
        all_loss_types = ['improved']
        datatype = 'animalkingdom_split'  # 'wolf', 'horse', 'animalkingdom'
